...
export const translations = {
  ...
  en: {
    ...
    'careers.build.card1.desc': 'Today Émile coordinates 4 specialist agents (auth, IDOR, injection, business logic) on every webapp pentest. Your job: extend to 8-12 specialist agents, ship the supervisor\'s memory layer so it stops re-asking the same questions on long horizons, improve the termination oracle so a 6-hour run knows when it\'s done.',
    'careers.build.card1.stack': 'Stack: Python, GPT-4o-mini and 120B-class open-weight on Scaleway H100, Coverage Graph (custom hierarchical data structure), AWS Bedrock for fine-tune.',
    'careers.build.card2.title': 'Offensive ML benchmark and eval harness',
    'careers.build.card2.desc': 'The agent stack is only as good as how you measure it. Gabriel built v0 of the benchmark VMs (intentionally vulnerable targets with known findings). Your job: industrialize it. Production-grade eval harness, regression detection per agent version, anti-cheat. Your weekly job becomes "did this commit regress us against the 200-finding ground-truth set."',
    'careers.build.card2.stack': 'Stack: Python, Docker, custom IaC, Scaleway VMs, Triton or similar inference server, Postgres for finding ledger.',
    'careers.build.card3.title': 'Coverage Graph and termination oracle',
    'careers.build.card3.desc': 'The Coverage Graph is the hierarchical data structure that tracks which endpoints, params, and auth contexts have been tested. Without it, agents loop forever and CISOs don\'t trust the result. Your job: make it the system of record for "is this app done." Includes graph merge across pentests, gap-detection heuristics, exhaustiveness scoring that converts cleanly to an audit PDF.',
    'careers.build.card3.stack': 'Stack: TypeScript or Python, graph DB or relational with adjacency lists, custom visualizer.',
    'careers.build.card4.title': 'Production infra (~10 concurrent pentests by Q4 2026)',
    'careers.build.card4.desc': 'Today Augustin reviews each pentest manually (~1.5 to 2 hours human-in-loop). The gate to scale is full automation plus 10 concurrent runs by Q4 2026. Your job (if infra is your wedge): GPU pool management on Scaleway, orchestration queue, observability that catches a stuck agent before the customer notices.',
    'careers.build.card4.stack': 'Stack: Scaleway H100, Kubernetes, Pulumi or Terraform, OpenTelemetry, Grafana.',
    ...
    'careers.emile.p2': 'When GPT-5 ships, our agents get smarter. When GPT-6 ships, our agents get smarter. When open-weight catches up (Kimi K2.5, gpt-oss-120b), we swap models in production with a config change. The model is the commodity layer. The orchestration plus Coverage Graph plus termination oracle plus audit-PDF generator is the moat. That\'s what we own. That\'s where the 98% of customer-perceived value lives.',
    ...
    'careers.positions.role1.desc': 'Extend Émile from 4 to 12 specialist agents. Build the memory and termination oracle.',
    ...
    'careers.positions.role3.background': 'Likely background: 5+ years shipping production systems end-to-end. You\'d be employee #4 (after Yanis, Augustin, Gabriel).',
